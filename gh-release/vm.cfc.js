...
var uncurryThis$f = NATIVE_BIND$2 && bind$2.bind(call$b, call$b);
...
var ordinaryToPrimitive$1 = function (input, pref) {
  var fn, val;
  if (pref === 'string' && isCallable$a(fn = input.toString) && !isObject$8(val = call$a(fn, input))) return val;
  if (isCallable$a(fn = input.valueOf) && !isObject$8(val = call$a(fn, input))) return val;
  if (pref !== 'string' && isCallable$a(fn = input.toString) && !isObject$8(val = call$a(fn, input))) return val;
...
function isCFC() {
  return global.IS_CFC || 'true' === 'true';
}
function isAGC() {
...
function setCron(time = 60000, slsType) {
  time = time || 60000;
...
const JSON5 = json5__namespace.default ? json5__namespace.default : json5__namespace;
...
var getIterator$1 = function (argument, usingIterator) {
  ...
  if (aCallable$3(iteratorMethod)) return anObject$2(call$4(iteratorMethod, argument));
...
async function printVersion() {
  ...
  let version = 'v0.5.220728';

  if (version.includes('.')) {
    logger.info(`当前版本【v0.5.220728】`);
  } else {
    version = undefined;
  }
...
async function request$1(reqFunc, options, ...args) {
  options || (options = {});
...
function getIdFuncArray() {
  ...
  const arr = [getAidByCustomizeUp, getAidByFollowing, getAidBySpecialFollowing, getAidByRegionRank];

  if (((_TaskConfig$coin$cust = TaskConfig.coin.customizeUp) === null || _TaskConfig$coin$cust === void 0 ? void 0 : _TaskConfig$coin$cust.length) === 0) {
    arr.shift();
  }
...
async function getTodayCoinNum() {
...
async function setCoinsTask() {
  ...
  coinNum || (coinNum = TaskConfig.coin.todayCoins);
...
function coinOtherHandle(id, state, coinData) {
  state.eCount++;
...
async function bigPointTask(taskStatus, isRetry = false) {
  var _task_info$sing_task_;

  const {
    task_info
  } = taskStatus;
  const signCode = await sign((_task_info$sing_task_ = task_info.sing_task_item) === null || _task_info$sing_task_ === void 0 ? void 0 : _task_info$sing_task_.histories, isRetry);
...
async function sign(histories, isRetry = false) {
...
async function liveMobileHeart$1(heartbeatParams, countRef, needTime = 75) {
  ...
  try {
    ...
    countRef.value++;
    liveLogger$1.info(`直播间心跳成功 ${heartbeatParams.uname}（${countRef.value}/${needTime}）`);
  } catch (error) {
...
async function activityLotteryService() {
  ...
  const list = uniqueObjectArray(netList.concat(userList.filter(item => expiredIdsFilter(item, localStatus)) || []), 'sid');
...
async function getActivityList(localStatus) {
  ...
  try {
    ...
    if (!isArray(reslut)) {
      return (localStatus === null || localStatus === void 0 ? void 0 : localStatus.activity_list) || [];
    }
...
function writeStatus(oldData, activityList = []) {
  ...
  try {
    ...
    if (EXPIRED_LIST.length) {
      pushIfNotExist(EXPIRED_LIST, ...((oldData === null || oldData === void 0 ? void 0 : oldData.expired_list) || []));
    ...
    fs.writeFileSync(FILE_PATH, JSON.stringify(deepMergeObject(oldData, activity_json), null, 2));
  } catch (error) {
    logger.debug(error);
  }
}
...
async function runMode1(errRef) {
  try {
    const {
      code,
      data,
...
async function runMode2(errRef, caseIdList) {
  try {
    const {
      code,
      data,
...
async function handleNoNewCase(message) {
...
async function handleNoNewCaseForMode2(message, caseIdList, errRef) {
  juryLogger.info(`${message}`);
